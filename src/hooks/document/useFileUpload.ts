...
export default function useFileUpload(callbacks: FileUploadCallbacks) {
  ...
  const initialFileList = (selectedFiles: File[]) => {
    const uploadFiles = selectedFiles.map((file) => ({
      file,
      fileName: file.name,
      fileType: file.name.split('.')[file.name.split('.').length - 1] as (typeof allowedExtensions)[number],
      fileSize: String(file.size),
      status: 'waiting' as const,
      uploadTime: getCurrentTime(),
      id: createFileHash(),
      creator: '当前登录人',// 当前登录人，****************联调之后，需要传入当前登录人**************//
  ...
  const checkFiles = (uploadFiles: UploadFile[]): Ref<{ validFiles: UploadFile[]; invalidFilesTypeMessage: string; invalidFilesSizeMessage: string }> => {
    const invalidFilesType: string[] = uploadFiles.filter((fileItem) => {
      return !allowedExtensions.includes(fileItem.fileType!)
    ...
    const validFiles = uploadFiles.filter((fileItem) =>{
      return  !invalidFilesType.includes(fileItem.fileName) 
        && !invalidFilesSize.includes(fileItem.fileName)
    })
  ...
  const processQueue = async () => {
    ...
    while (activeUploads.value.size < MAX_CONCURRENT_UPLOADS && uploadQueue.value.length > 0) {
      ...
      handleUpload(task).finally(async () => {
        // 从正在上传的文件集合中移除当前文件
  ...
  const handleUpload = async (task: UploadFile) => {
    try{
      ...
      const res = await uploadDocument(task.file!,task.uploadController)

      const { failed_uploads, successful_uploads, files_map } = res.data.data;

      //修改状态
      const newStatus = successful_uploads.length > 0 ? 'parsing' : 'failed';
      callbacks.updateItemById(task.id, {status:newStatus});

      // 成功的话，将初始前端设置的文件id改为上传成功后正式的文件ID
      if(successful_uploads.length > 0){
        // 传回上级，更新表格数据
        callbacks.updateItemById(task.id, { id: files_map[task.fileName] });
      }

      // 如果上传失败，则提示失败原因
      if(failed_uploads.length > 0){
        const message = files_map[task.fileName] || '';
        if(message){
