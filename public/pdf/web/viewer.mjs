...
class GetPageFullText {
  constructor() {}
  async getFullText(page) {
    return page.getTextContent( {disableNormalization: true}).then(textContent => {
      ...
      // console.log(strBuf)
      // console.log(normalize(strBuf.join("")))
      // [this._pageContents[i], this._pageDiffs[i], this._hasDiacritics[i]] =
        const res = normalize(strBuf.join(""));
        return res[0]
      // console.log('this._pageContents[i]', this._pageContents[i])
   
    }, reason => {
      console.error(`Unable to get text content for page`, reason);
...
const PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    this.getPageFullText = new GetPageFullText();
